...
def _add_optimization_parser(parser):
    ...
    o_parser.add_argument('--K', type=int, default=5,
                          help="K for alig+ optimiser")
...
def _add_loss_parser(parser):
    ...
    l_parser.add_argument('--weight_decay', "--wd", type=float, default=0,
                          help="weight decay amount")
    l_parser.add_argument('--l2_reg', type=float, default=0,
                          help="l2-regularization")
    l_parser.add_argument('--max_norm', type=float, default=None,
                          help="maximal l2-norm for constrained optimizers")
    l_parser.add_argument('--loss', type=str, default='ce', choices=("svm", "ce", "map", "norm_ce", "mse", "log"),
                          help="loss function to use ('svm' or 'ce' or 'map' or 'mse')")
...
def _add_misc_parser(parser):
    ...
    m_parser.add_argument('--port', type=int, default=9003,
...
def set_xp_name(args):
    ...
    if args.xp_name is None:
        ...
        xp_name += "{model}{data}-{opt}--eta-{eta}--l2-{l2}--b-{b}-{tag}"
        l2 = args.max_norm if ('alig' in args.opt and not(args.weight_decay or args.l2_reg)) or (args.opt == 'sbd') else args.weight_decay or args.l2_reg
        data = args.dataset.replace("cifar", "")
        xp_name += "--momentum-{}".format(args.momentum)
        args.xp_name = xp_name.format(model=args.model, data=data, opt=args.opt, eta=args.eta, l2=l2, b=args.batch_size, tag=args.tag)
        if args.debug:
            args.xp_name += "--debug"
...
def set_num_classes(args):
    if args.dataset == 'cifar10':
        ...
        args.n_batches_per_epoch = 45000//args.batch_size + int(not(45000%args.batch_size==0))
        ...
        args.n_classes = 10
        args.n_batches_per_epoch = 598388//args.batch_size + int(not(598388%args.batch_size==0))
    elif args.dataset == 'imagenet':
        args.n_batches_per_epoch = 1231166//args.batch_size + int(not(1231166%args.batch_size==0))
        args.n_classes = 1000
        args.input_dims = 3*224**2
    elif args.dataset == 'tiny_imagenet':
        args.n_batches_per_epoch = 100000//args.batch_size + int(not(100000%args.batch_size==0))
        args.n_classes = 200
